...
export default {
    ...
    computed: {
        ...
        getTotalSummaryFields() {
            ...
            var { hide_empty } = this.getSettings?.general

            var fields = Object.values(calcStore)
                .filter(field => !['html', 'line', 'text'].includes(field.alias.replace(/\_field_id.*/,'')));

            /** remove zero value fields **/
            if ( hide_empty && hide_empty === 'hide' ) {
    ...
    methods: {
        ...
        init() {
            ...
            if ( typeof this.calc_data !== "undefined" && this.calc_data && this.calc_data.hasOwnProperty('fields' ) ) {
                ...
                if ( this.calc_data.hasOwnProperty('styles' ) && this.calc_data.styles ) {
                ...
                if ( this.calc_data.customs ) {
                    this.customs = this.calc_data.customs
                }
        ...
        setStoreNumberTypeFields( element ){
            ...
            const len = this.currencySettings?.num_after_integer || 2;

            let value = element.value ? element.value : 0;
            value = element.unit ? this.validateUnit(value * element.unit, len) : value;
            ...
            this.filterUnused(extra, element);
            this.$set(this.calcStore, element.alias, {
                extra,
                value,
                unit,
                slideValue,
        ...
        setStoreSingleControlFields( element ){
            ...
            if ( element.value && element.value.length > 0 ) {
                var fieldToData = vm.calc_data.fields.filter(function(item) { return item.alias === element.alias })[0];
            ...
            this.formula
                .forEach( element => {
                    ...
                    if ( totalSymbol && typeof totalSymbolSign !== 'undefined') {
                        element.converted = this.currencyFormat(element.total, {currency: true}, {...this.currencySettings, currency: totalSymbolSign})
                    } else {
    ...
    filters: {
        ...
        to_short(value, container, len = 40) {
            if (container === 'v-container' && value.length >= len) {
