	...
	<div class="existing-wrapper">
		<div class="existing-list header">
			...
			<div class="list-title">
				<a @click.prevent="$store.commit('updateIsExisting', true)"><i class="fas fa-arrow-circle-left"></i> &nbsp; <?php esc_html_e( 'Go back', 'cost-calculator-builder' ); ?>
				</a>
			</div>
		</div>
		<div class="existing-list content">
			<div v-if="demoImport.finish" class="panel-custom p-t-30 p-b-30 text-center">
				<h2><?php esc_html_e( 'Finish :)', 'cost-calculator-builder' ); ?></h2>
			</div>
			<div v-else>
				<div class="p-t-15">
					<div class="text-center p-t-15 p-b-15" v-if="demoImport.step_progress">
						<h4> <?php esc_html_e( 'Import step progress', 'cost-calculator-builder' ); ?> :
							<strong>{{ demoImport.step_progress }}</strong>
						</h4>
					</div>

					<div v-if="demoImport.progress_load" class="progress">
						<div class="progress-bar progress-bar-striped progress-bar-animated bg-success" role="progressbar" :aria-valuenow="demoImport.progress" aria-valuemin="0" aria-valuemax="100" :style="'width: '+demoImport.progress+'%'">
							{{ demoImport.progress }}%
						</div>
					</div>
