...
class STM_Admin_Notification {
	...
	public static function init( $plugin_data ) {
		...
		add_filter(
			'stm_set_admin_notices_data',
			function ( $notices ) use ( $notices_data ) {
	...
	public static function stm_admin_notices() {
		...
		$notice_data = apply_filters( 'stm_set_admin_notices_data', array() );
	...
	public static function stm_admin_notice_template( $plugin_name, $event = '' ) {
		if ( ! empty( $event ) ) {
			$data_single       = apply_filters( 'stm_admin_notice_rate_' . $plugin_name . '_' . $event, array() );
	...
	public static function admin_enqueue() {
		wp_enqueue_style( 'stm_admin_notice_rate_css', STM_ADMIN_NOTIFICATION_URL . '/assets/css/rate.css', false ); //phpcs:ignore
	...
	public static function ajax_admin_notice() {
		...
		if ( isset( $_POST['type'] ) && isset( $_POST['pluginName'] ) && isset( $_POST['pluginEvent'] ) ) {
			...
			if ( empty( $notice ) || ( 'single' !== $plugin_event && ! isset( self::$conditions[ $type ] ) ) ) {
				return;
			}

			$step = $notice['step'];

			if ( ( 'single' === $plugin_event ) || ( ! empty( $notice['prev_action'] ) && ! isset( self::$conditions[ $notice['prev_action'] ][ $step ] ) ) ) {
				$period = 0;
			} else {
