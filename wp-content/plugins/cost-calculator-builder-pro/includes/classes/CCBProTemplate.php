...
class CCBProTemplate {
	...
	public static function plugin_path( $default_path = '' ) {
		...
		$version_control = empty( get_option( 'ccb_version_control' ) ) ? 'v1' : get_option( 'ccb_version_control' );
		return untrailingslashit( CCB_PRO_PATH . '/templates/' . $version_control . '/' ); // phpcs:ignore
	}
}
